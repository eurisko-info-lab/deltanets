...
import { D, Node2D } from "@deltanets/render";
import { MethodState } from "../index.ts";
import { type Graph, type NodePort, type Redex } from "@deltanets/core";
import { type Data, getRole } from "./config.ts";
...
export const renderNodePort = (
  ...
  switch (role) {
    ...
    case "binder":
      if (nodePort.port === 0) {
        return renderBinderAgent(nodePort, state, redexes, level);
      }
      break;
    case "applicator":
      if (nodePort.port === 1) {
        return renderApplicatorAgent(nodePort, state, redexes, level);
      }
      break;
    case "type-constructor":
      if (nodePort.port === 0) {
        return renderTypeConstructorAgent(nodePort, state, redexes, level);
      }
      break;
    case "destructor":
      if (nodePort.port === 1) {
        return renderDestructorAgent(nodePort, state, redexes, level);
      }
      break;
    case "replicator":
      if (nodePort.port !== 0) {
        return renderReplicatorInAgent(nodePort, state, redexes, level);
      }
      if (nodePort.port === 0) {
        return renderReplicatorOutAgent(nodePort, state, redexes, level);
      }
      break;
  }
